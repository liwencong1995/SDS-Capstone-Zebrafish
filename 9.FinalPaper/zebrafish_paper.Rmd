...
# Data and Variables
```{r fig.cap = "\\label{landmark} Landmark", fig.align='center', echo=FALSE}
knitr::include_graphics("figures/landmark.png", dpi = 260 )
```

We have 43 wildtypes samples ($n_1$) and 35 mutant samples ($n_2$) for training and testing. There are 152 landmarks ($N$) for each sample, which resemble Fig \ref{landmark}, with each of them containing the following variables:
...
# Support Vector Machine 
The goal of SVM is to find a separation line $f(x) = (\beta_0 + \beta_1 \cdot x_1 + \beta_2 \cdot x_2)$ that separates the nearest data as cleanly as possible as shown in Fig \ref{svmfig} below. The parameters $\beta$ are found by solving the optimization problem --to maximize Margin subject to some restrictions -- in 2 dimensions [@James2013].
...
## Cross-Validation
For our project, we have access to 43 wild-type samples and 35 mutant-type samples. Due to this limited sample size, we decided to use a leave-one-out cross validation method to test our model. For each testing sample, we built 152 SVMs for each landmark.
...
Fig \ref{inputdata} shows a ramdom sample of an input file for the script `svm.py`. The columns `stype` (sample type), `landmark_index` and `sample_index` are arequired and the other two columns (`pts` and `r` in this example) are the two parameters for building SVM.
...
Fig \ref{shiny2} displays the Precision Score Visualization tab of the first sample of AT channel. In this case, all three thresholds are at default level, 0. Therefore, all landmarks' precision scores are shown in both the histogram and the heatmap.
