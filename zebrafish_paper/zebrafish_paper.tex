...
distance of all points in each wedge, R. We use number of points in each
wedge and median R to run SVM models to do classifications.
...
recognition problems. The goal of SVM is to find a hyperplane Yi(b0 +
b1Xi1 + b2Xi2) = 0 that separates the nearest data as clean as possible.
The parameters bi are found by solving the optimization problem below.
...
\(Y_i(b0 + b1Xi1 + b2Xi2) = 0\) that separates the nearest data as clean
as possible. The parameters bi are found by solving the optimization
problem below.
